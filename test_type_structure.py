...
async def test_type_structure_controllers():
    ...
    async with ASFConnector(
        ...
        test_types = ['ArchiSteamFarm.Steam.Storage.BotConfig', 'ArchiSteamFarm.Storage.GlobalConfig']
        ...
        for type_name in test_types:
            ...
            try:
                ...
                if result.get('Success'):
                    print(f"  ✓ Type information retrieved successfully")
        ...
        test_structures = ['ArchiSteamFarm.Steam.Storage.BotConfig', 'ArchiSteamFarm.Storage.GlobalConfig']
        ...
        for structure_name in test_structures:
            ...
            try:
                ...
                if result.get('Success'):
                    print(f"  ✓ Structure information retrieved successfully")
                    # Show a preview of the result
                    result_data = result.get('Result', {})
                    if isinstance(result_data, dict):
                        ...
                        if len(result_data) > 3:
                            print(f"      ... and {len(result_data) - 3} more properties")
                    else:
                        print(f"    Result: {str(result_data)[:100]}...")
                else:
                    print(f"  ✗ Failed: {result.get('Message', 'Unknown error')}")
