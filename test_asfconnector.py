...
class PersistenceTest(unittest.TestCase):
    ...
    def test_performance_comparison(self):
        ...
        print(f"  Legacy API (no pool):     {legacy_time:.3f}s ({legacy_time/num_requests*1000:.1f}ms per request)")
        print(f"  New Controller (no pool): {new_time:.3f}s ({new_time/num_requests*1000:.1f}ms per request)")
        print(f"  New Controller (w/ pool): {pool_time:.3f}s ({pool_time/num_requests*1000:.1f}ms per request)")
        print(f"\nSpeedup with connection pool: {(new_time/pool_time):.2f}x faster")
        print(f"Time saved per request: {(new_time-pool_time)/num_requests*1000:.1f}ms")
        print("="*60)
